...
function buildHtmlList(itemSeries) {
  ...
  const html = `<li class="js-series ${
    isFav !== undefined ? favClass : ""
  } csseditseries" id="${itemSeries.show.id}">
                    <div class = "container">
                        <div class = "image">
                            <img class="seriesImage " src="${
                              itemSeries.show.image !== null
                                ? itemSeries.show.image.medium
                                : defaultImagePath
                            }" alt="series image"/>
                            <h2 class="csstextseries ">${
                              itemSeries.show.name
                            }</h2>
                        </div>
                        <div class = "overlay">
...
function buildHtmlListFav(itemSeries) {
  ...
  let html = `<div class="flexboxfav">
                    <li class="js-series ${favSecondClass}" id="${
    itemSeries.show.id
  }">
    ...
    itemSeries.show.image !== null
      ...
      : defaultImagePath
                    ...
                    <button class="closeButton js-series-close" id="${
                      itemSeries.show.id
                    }">
                    <button class="js_countfavs Favorite List" id="${
                      itemSeries.show.id
                    }">
...
//     if (
//       language == serieRecomend[0] ||
//       language == serieRecomend[1] ||
//       language == serieRecomend[2]
//     ) {
...
function cleanLS() {
  ...
  let buttonConfirm = confirm(
    "¿Está seguro de que desea eliminar toda la lista de favoritos?"
  );
